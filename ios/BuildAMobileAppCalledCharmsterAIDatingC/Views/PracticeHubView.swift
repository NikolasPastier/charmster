...
struct PracticeHubView: View {
                    ...
                    ForEach(PracticeSetting.library) { s in
                        Button {
                            ...
                            .overlay(RoundedRectangle(cornerRadius: 12)
                                .stroke(!useCustom && setting.id == s.id ? Theme.accent : Theme.border,
                    ...
                    HStack(spacing: 10) {
                        ForEach(PartnerPersona.library) { p in
                            Button { persona = p } label: {
                                ...
                                .background(RoundedRectangle(cornerRadius: 12).fill(Theme.surfaceRaised))
                                .overlay(RoundedRectangle(cornerRadius: 12)
                                    .stroke(persona.id == p.id ? Theme.accent : Theme.border,
    ...
    private var sandboxModeCard: some View {
        GlassCard {
            HStack(spacing: 10) {
                Button { coached = true } label: {
                    sandboxChip("Coached", "Tips + scoring", on: coached)
                }
                Button { coached = false } label: {
    ...
    private var startCard: some View {
        VStack(spacing: 10) {
            ...
            AuraButton(title: "Start session", systemImage: "play.fill",
...
private struct SandboxRunner: View {
    ...
    var body: some View {
        ZStack {
            if let r = result {
                ResultsView(result: r, lecture: nil, onQuiz: {}, onDone: {
                    dismiss(); onClose(r)
                })
            } else {
                LivePracticeView(
