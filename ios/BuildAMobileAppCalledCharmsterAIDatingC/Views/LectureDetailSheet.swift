...
struct LectureDetailSheet: View {
    ...
    enum Route { case lecture, configurator, practice(SessionConfig), results(SessionResult), quiz }
    ...
    private var narratorBar: some View {
        GlassCard(padding: 14) {
            HStack(spacing: 12) {
                Button {
                    if narrator.isSpeaking { narrator.pauseOrResume() }
                    else { narrator.speak(content.narrationScript, coach: app.coachMode) }
                } label: {
    ...
    private func resultsScreen(result: SessionResult) -> some View {
        ResultsView(result: result, lecture: lecture,
                    onQuiz: { route = .quiz },
                    onDone: { dismiss() })
    }
    ...
    private var quizScreen: some View {
        QuizView(lecture: lecture, onDone: { correct in
