...
struct AuraButton: View {
  ...
  private var gradient: LinearGradient {
    ...
    case .coral:
      return LinearGradient(
        colors: [Theme.coral, Color(hex: 0xB73B40)],
  ...
  var body: some View {
    Button(action: action) {
      ...
      .shadow(
        color: (tone == .accent ? Theme.pink : (tone == .coral ? Theme.red : Theme.gold))
          .opacity(enabled ? 0.45 : 0), radius: 18, y: 8
      )
      .shadow(
        color: (tone == .accent ? Theme.purple : (tone == .coral ? Theme.red : Theme.orange))
