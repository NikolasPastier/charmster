...
final class CurriculumService {
    ...
    private func runFetch() async {
        ...
        do {
            ...
            TenXPreviewSupport.log("[Curriculum] overlaid \(t.count) tracks · \(l.count) lectures from Supabase")
        } catch {
            TenXPreviewSupport.log("[Curriculum] fetch failed: \(error.localizedDescription) — using bundle")
        }
    }
    ...
    private func fetchTracks(base: URL, anon: String) async throws -> [Track] {
        let url = base
            ...
            .appending(queryItems: [
                URLQueryItem(name: "select", value: "id,slug,emoji,title,subtitle,core_question,symbol,order,access_default"),
    ...
    private func fetchLectures(base: URL, anon: String) async throws -> [Lecture] {
        let url = base
            ...
            .appending(queryItems: [
                URLQueryItem(name: "select", value: "id,track_id,lecture_number,title,scenario,access,format,minutes,skill,is_capstone"),
    ...
    private func get(_ url: URL, anon: String) async throws -> Data {
        ...
        if let http = resp as? HTTPURLResponse, !(200..<300).contains(http.statusCode) {
            throw NSError(domain: "Curriculum", code: http.statusCode,
