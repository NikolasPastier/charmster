...
final class RealtimeLiveSession: NSObject {
    ...
    enum Status { case idle, connecting, live, ended, failed(String) }
    ...
    private func startAudio() {
        ...
        let outFmt = AVAudioFormat(commonFormat: .pcmFormatInt16, sampleRate: 24_000,
    ...
    private func handleMicBuffer(_ buf: AVAudioPCMBuffer) {
        ...
        let outCapacity = AVAudioFrameCount(Double(buf.frameLength) * outFmt.sampleRate / buf.format.sampleRate) + 1024
    ...
    private func enqueuePartnerAudio(_ pcm: Data) {
        ...
        if lastResponseEnergies.count > 64 { lastResponseEnergies.removeFirst(lastResponseEnergies.count - 64) }
    ...
    private func rmsLevel(_ buf: AVAudioPCMBuffer) -> Double {
        ...
        if let ch = buf.floatChannelData {
            let p = ch[0]
            for i in 0..<count {
                let v = Double(p[i]); sumSq += v * v
            }
        } else if let ch = buf.int16ChannelData {
            let p = ch[0]
            for i in 0..<count {
                let v = Double(p[i]) / 32768.0; sumSq += v * v
            }
        }
...
private extension Double {
    ...
    var normalized: Double { max(0, min(1, self * 6)) }
}
