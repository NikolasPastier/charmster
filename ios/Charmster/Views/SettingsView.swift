...
struct SettingsView: View {
    ...
    private var profileSection: some View {
        Section("Profile & account") {
            ...
            HStack { Text("Profile picture"); Spacer()
                Image(systemName: "person.crop.circle.fill")
                    .font(.system(size: 22)).foregroundStyle(Theme.textMuted) }
            HStack { Text("Sign-in method"); Spacer(); Text("Apple").foregroundStyle(Theme.textMuted) }
            HStack { Text("Email"); Spacer(); Text("alex@example.com").foregroundStyle(Theme.textMuted) }
            HStack { Text("Age"); Spacer(); Text("17+").foregroundStyle(Theme.textMuted) }
    ...
    private var personalizationSection: some View {
        Section("Personalization") {
            Picker("Goal", selection: bindingFor(\.profile.goal, then: { app.recomputePersonalization() })) {
                ForEach(["Date with intention", "Date casually", "Get unstuck", "Confidence in general"], id: \.self) {
                    Text($0).tag($0)
                }
            }
            Picker("Experience", selection: bindingFor(\.profile.experience, then: { app.recomputePersonalization() })) {
                ForEach(["Brand new", "Some experience", "Plenty, but mixed", "Veteran"], id: \.self) {
                    Text($0).tag($0)
                }
            }
            Picker("Flirting style", selection: bindingFor(\.profile.flirtingStyle, then: { app.recomputePersonalization() })) {
                ForEach(["Warm", "Playful", "Dry", "Direct"], id: \.self) { Text($0).tag($0) }
            }
            Stepper(value: bindingFor(\.profile.confidence, then: { app.recomputePersonalization() }), in: 1...10) {
                HStack { Text("Confidence"); Spacer(); Text("\(app.profile.confidence)/10").foregroundStyle(Theme.textMuted) }
            }
    ...
    private var coachingSection: some View {
        Section("Coaching & difficulty") {
            ...
            HStack {
                ...
                Text(app.profile.feedbackGentleness > 0.6 ? "Gentle" :
                     (app.profile.feedbackGentleness < 0.4 ? "Direct" : "Mid"))
    ...
    private var learningGoalsSection: some View {
        Section("Learning goals & reminders") {
            ...
            HStack { Text("Streak freeze / rest days"); Spacer(); Text("2 left").foregroundStyle(Theme.textMuted) }
        }
    }
    ...
    private var notificationsSection: some View {
        Section("Notifications") {
            ...
            DatePicker("Quiet hours start",
                       selection: Binding(get: { app.profile.quietHoursStart ?? .now },
                                          set: { app.profile.quietHoursStart = $0 }),
                       displayedComponents: .hourAndMinute)
            DatePicker("Quiet hours end",
                       selection: Binding(get: { app.profile.quietHoursEnd ?? .now },
    ...
    private var membershipSection: some View {
        Section("Membership") {
            HStack { Text("Current plan"); Spacer()
                Text(planLabel).foregroundStyle(Theme.textMuted) }
            HStack { Text("Today's sessions"); Spacer()
    ...
    private var supportSection: some View {
        Section("Support & legal") {
            ...
            HStack { Text("Version"); Spacer(); Text("1.0.0").foregroundStyle(Theme.textMuted) }
            HStack { Text("Age rating"); Spacer(); Text("17+").foregroundStyle(Theme.textMuted) }
        }
    }
    ...
    private var dangerZoneSection: some View {
        ...
        .confirmationDialog("Delete account? This removes your profile, progress, sessions, and membership locally.",
    ...
    private func bindingFor<T>(_ key: ReferenceWritableKeyPath<AppState, T>,
                               then: (() -> Void)? = nil) -> Binding<T> {
        Binding(get: { app[keyPath: key] },
                set: { app[keyPath: key] = $0; then?() })
    }
...
private struct FocusAreasView: View {
    @Environment(AppState.self) private var app
    private let options = ["Opening", "Flow", "Calibration", "Frame", "Closing", "Repair", "Presence"]
    var body: some View {
        List {
        ...
        Form {
            ...
            Button("Save") {
                ...
                app.profile.attachmentLabel =
                    anxiety > 0.65 ? "Anxious-leaning" :
                    avoidance > 0.65 ? "Avoidant-leaning" : "Secure-leaning"
                app.recomputePersonalization()
            }
            .foregroundStyle(Theme.accent)
        }
...
private struct CancelSurveyView: View {
    ...
    private let reasons = ["Too expensive", "Not using it enough", "Found something better",
