...
private struct IntroCarouselView: View {
  ...
  private static let newLogoURL = URL(
    string:
  ...
  var body: some View {
    ZStack {
      ...
      VStack(spacing: 0) {
        TabView(selection: $page) {
          ForEach(slides.indices, id: \.self) { i in
            VStack(spacing: 18) {
              ...
              AsyncImage(url: Self.newLogoURL, transaction: Transaction(animation: .smooth)) {
                phase in
                switch phase {
        ...
        AuraButton(
          title: page == slides.count - 1 ? "Start the quiz" : "Continue",
        ...
        ForEach(Array(questions[index].1.enumerated()), id: \.offset) { i, opt in
          Button {
            ...
            .overlay(
              RoundedRectangle(cornerRadius: 16, style: .continuous)
                .stroke(
                  answers[index] == i ? Theme.accent : Theme.border,
