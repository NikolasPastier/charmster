...
struct PracticeConfiguratorView: View {
        ...
        ScrollView(.horizontal, showsIndicators: false) {
          HStack(spacing: 10) {
            ForEach(PartnerPersona.library) { p in
              chip(
                title: p.displayName, sub: p.pronouns,
          ...
          ForEach(PracticeSetting.library) { s in
            Button {
              ...
              .overlay(
                RoundedRectangle(cornerRadius: 12)
                  .stroke(
                    cfg.setting.id == s.id ? Theme.accent : Theme.border,
          ...
          ForEach(DifficultyTier.allCases) { t in
            Button {
              ...
              .overlay(
                RoundedRectangle(cornerRadius: 12)
                  .stroke(
                    cfg.tier == t ? t.color : Theme.border,
        ...
        ScrollView(.horizontal, showsIndicators: false) {
          HStack(spacing: 10) {
            ForEach(CoachStyle.allCases) { c in
              chip(
                title: c.title, sub: c.blurb,
                selected: cfg.coach == c
              ) { cfg.coach = c }
            }
          }
        }
  ...
  private var sandboxModePicker: some View {
    GlassCard {
      VStack(alignment: .leading, spacing: 12) {
        ...
        HStack(spacing: 10) {
          sandboxChip(
            title: "Coached", sub: "Tips + scoring (½ Aura)",
            selected: cfg.sandboxScored
          ) { cfg.sandboxScored = true }
          sandboxChip(
            title: "Just Vibe", sub: "No tips, no scoring",
            selected: !cfg.sandboxScored
          ) { cfg.sandboxScored = false }
        }
      }
    }
  }

  private func chip(title: String, sub: String, selected: Bool, onTap: @escaping () -> Void)
    -> some View
  {
    Button(action: onTap) {
  ...
  private func sandboxChip(title: String, sub: String, selected: Bool, onTap: @escaping () -> Void)
    -> some View
  {
    Button(action: onTap) {
      VStack(spacing: 6) {
